...
class StockCardReportWizard(models.TransientModel):
    ...
    sh_company_id = fields.Many2one(
        comodel_name='res.company', string='Company',default=lambda self: self.env.user.company_id, required=True)
    ...
    sh_domain_warehouse = fields.Char(
        string='Domain warehouse', compute='_compute_location_domain', store=True)
    ...
    def sh_print_stock_report(self):
        ...
        datas = self.read()[0]
        res = self.env.ref(
            'stock_card_report.stock_card_report_action').report_action([], data=datas)
        return res
            ...
            for category in sh_category:
                ...
                if sh_products:
                    for product in sh_products:
                            ...
                            for order in sale_stock_order_open:
                                if order.location_id.id != order.location_dest_id.id :
                                    ...
                                    if order.location_id.id == self.sh_location_id.id:
                                        ...
                                        transfer -= qty_done
                        open_stock = round(onhand+transfer+pos_r_qty_open,2)
                        ...
                        if total_in_qty or total_out_qty or close_stock or lines_list:
                            product_dict[category.display_name].append(
                                product.display_name)
                            stock_card_dict[category.display_name][product.display_name] = {
                                'open_stock': round(open_stock,2), 'lines': lines_list, 'total': ['', '', round(total_in_qty,2), round(total_out_qty,2), round(close_stock,2)]}
                if len(product_dict[category.display_name]) > 0:
                    category_list.append(category.display_name)
                if not stock_card_dict[category.display_name]:
                    del stock_card_dict[category.display_name]
            ...
            if sh_products:
                for product in sh_products:
                    ...
                    if len(lines_list) < 1:
                        ...
                        close_stock = lines_list[-1][-1]
                    if open_stock or lines_list or total_in_qty or total_out_qty or close_stock:
                        product_list.append(product.display_name)
                        stock_card_dict[product.display_name] = {'open_stock': round(open_stock,2), 'lines': lines_list, 'total': [
                            '', '', round(total_in_qty,2), round(total_out_qty,2), round(close_stock,2)]}
            ...
            if self.sh_select_product_cat == 'category':
                ...
                for category in category_list:
                    ...
                    for product in product_dict[category]:
                        ...
                        line_var += 1
                        worksheet.write_merge(
                            line_var, line_var, 0, 3, 'Opening Balance', bold_text)
                        worksheet.write_merge(
                        ...
                        for line in stock_card_dict[category][product]['lines']:
                            ...
                            worksheet.write_merge(
                                line_var, line_var, 2, 2, round(line[2],2), normal_text)
                            worksheet.write_merge(
                                line_var, line_var, 3, 3, round(line[3],2), normal_text)
                            worksheet.write_merge(
                                line_var, line_var, 4, 4, round(line[4],2), normal_text)
                            line_var += 1
                        worksheet.write_merge(
                            line_var, line_var, 1, 1, 'TOTAL', bold_text)
                        worksheet.write_merge(
                            line_var, line_var, 2, 2, round(stock_card_dict[category][product]['total'][2],2), bold_text)
                        worksheet.write_merge(
                            line_var, line_var, 3, 3, round(stock_card_dict[category][product]['total'][3],2), bold_text)
                        worksheet.write_merge(
                            line_var, line_var, 4, 4, round(stock_card_dict[category][product]['total'][4],2), bold_text)
                        line_var += 2
            ...
            # Print Product wise xls report
                ...
                for product in product_list:
                    ...
                    worksheet.write_merge(
                        line_var, line_var, 1, 1, 'TOTAL', bold_text)
                    worksheet.write_merge(
                        line_var, line_var, 2, 2, round(stock_card_dict[product]['total'][2],2), bold_text)
                    worksheet.write_merge(
                        line_var, line_var, 3, 3, round(stock_card_dict[product]['total'][3],2), bold_text)
                    worksheet.write_merge(
                        line_var, line_var, 4, 4, round(stock_card_dict[product]['total'][4],2), bold_text)
                    line_var += 2
        else:
